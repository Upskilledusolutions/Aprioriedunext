...
function evaluate(p,names){if(!exists(p))return{};try{return new Function(`${clean(read(p))}\nreturn {${names.map(n=>`${n}:typeof ${n}!=="undefined"?${n}:null`).join(",")}};`)()}catch(e){fail.push(`Question bank could not be evaluated: ${p}: ${e.message}`);return{}}}
const banks=[["src/Data/Reasoning/questionBank.js",["REASONING_QUESTION_BANK"]],["src/Data/Reasoning/questionBankStage1Extensions.js",["STAGE1_EXTENSION_QUESTIONS"]],["src/Data/Reasoning/questionBankStage2.js",["STAGE2_QUESTIONS"]],["src/Data/Reasoning/questionBankStage3.js",["STAGE3_QUESTIONS"]],["src/Data/Reasoning/questionBankStage4.js",["questions"]],["src/Data/Reasoning/questionBankStage5.js",["STAGE5_QUESTION_BANK"]],["src/Data/Reasoning/questionBankStage6.js",["STAGE6_QUESTION_BANK"]],["src/Data/Reasoning/questionBankLevel2Stage1.js",["LEVEL2_STAGE1_QUESTION_BANK"]]];
let all=[];for(const [p,names] of banks){const data=evaluate(p,names);for(const n of names){const bank=data[n];if(!Array.isArray(bank)){fail.push(`Expected question array ${n} was not found in ${p}`);continue}if(bank.length===0)fail.push(`Question bank is empty: ${p}`);all.push(...bank.map(q=>({...q,__file:p})))} }
const ids=all.map(q=>q.id).filter(Boolean);if(new Set(ids).size!==ids.length)fail.push("Duplicate Reasoning question IDs detected across the audited banks.");
for(const q of all)if(!q.id||!q.activityId||!q.question||!Array.isArray(q.options)||q.options.length!==4||!q.options.includes(q.answer))fail.push(`Malformed question record: ${q.id||"unknown"}`);
function textOnly(q){return q.options.every(o=>typeof o==="string"&&!/[0-9₹$€£%]/.test(o))}
