...
import { data } from "../../Data/Speechtracker/English"; // Importing the quiz data
...
export default function FrenchQuizes() {
  ...
  return (
    <>
      <Head>
        <title>English Reading Practice (A1)</title>
      ...
      <main>
        <div className={styles.container}>
          <div className={styles.headcont}>
            <div className={styles.mainheading}>English Reading Practice (A1)</div>
          </div>
            ...
            {data.map((data, index) => {
              ...
              return (
                <div key={data.quiz} className={`${styles.card1} ${isLocked ? styles.locked : ''}`}>
                  {isUnlocked || user.type === 'all' ? <Link href={`EnglishReadingPage/${data.id}`} className={styles.link}>
                  {isLocked && (
