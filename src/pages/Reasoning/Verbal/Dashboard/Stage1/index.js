...
export default function FoundationVerbal() {
  ...
  const [progress, setProgress] = useState({ verbal: { completedModules: {}, scores: {} } });
    ...
    <>
      ...
      <main style={styles.page}><div style={styles.wrapper}>
        ...
        <div style={styles.grid}>{modules.map(([number, title, text, action], index) => {
          ...
          return <article key={number} style={styles.card}><div style={styles.number}>MODULE {number}</div><h2 style={styles.cardTitle}>{title}</h2><p style={styles.text}>{text}</p>{complete && <span style={styles.status}>Completed · Score {progress.verbal.scores[`module${index + 1}`]}/5</span>}<Link href={`/Reasoning/Verbal/Dashboard/Stage1/Module${index + 1}`} style={styles.button}>{complete ? "Review Module" : action}</Link></article>;
        })}</div>
        <p style={styles.note}>Your Verbal progress is stored separately from the Foreign Languages product and is ready to be connected to backend persistence later.</p>
      </div></main>
    </>
  );
}
