...
export default function ReasoningActivity() {
  ...
  const moduleId = activity?.track === "quantitative" ? ({ "arithmetic-reasoning":"Module1", "number-patterns":"Module2", "logic-puzzles":"Module3", "shape-measurement":"Module4", "multi-step-reasoning":"Module5" }[activity.moduleId]) : ({ "main-idea":"Module1", "sequencing":"Module2", "evidence-and-claims":"Module3", "inference":"Module5", "perspectives":"Module5" }[activity?.moduleId]);
  const module = useMemo(() => activity && moduleId ? getStage1Module(activity.track, moduleId) : null, [activity, moduleId]);
  ...
  useEffect(() => { if (!router.isReady) return; if (!isAuthenticated || !user?.userId) router.replace({ pathname: "/Auth", query: { redirect: router.asPath } }); }, [router.isReady, router, isAuthenticated, user]);
  useEffect(() => { if (!question || isAnswered || isExpired) return undefined; setRemaining(timeLimit); const timer = window.setInterval(() => { setRemaining((value) => { if (value <= 1) { window.clearInterval(timer); setExpired((current) => ({ ...current, [question.id]: true })); return 0; } return value - 1; }); }, 1000); return () => window.clearInterval(timer); }, [question?.id, timeLimit, isAnswered, isExpired]);
  ...
  return <><Head><title>{activity.title} | Reasoning</title></Head><main style={{ minHeight:"80vh",padding:"48px 20px 80px",background:"var(--surface)" }}><div style={{ maxWidth:850,margin:"0 auto" }}>
    ...
    {!completed ? <section style={{ marginTop:30,padding:28,borderRadius:18,background:"var(--card)",border:"1px solid var(--border)" }}><div style={{ display:"flex",justifyContent:"space-between",gap:16,flexWrap:"wrap",color:"var(--muted)",fontSize:14,fontWeight:700 }}><span>Question {currentIndex+1} of {questions.length}</span><span>{answeredCount} answered · {questions.length-handledCount} remaining</span><span>Score: {score}%</span></div><div style={{ height:8,background:"var(--border)",borderRadius:99,marginTop:14,overflow:"hidden" }}><div style={{ width:`${progressPercent}%`,height:"100%",background:"var(--blue)" }}/></div><div style={{ marginTop:18,padding:"10px 14px",borderRadius:10,background:"rgba(0,59,147,.07)",fontWeight:800 }}>{isExpired?"Time expired":isAnswered?"Answer recorded":`Time left: ${remaining}s`}</div><h2 style={{ fontSize:24,lineHeight:1.4,marginTop:24 }}>{question.question}</h2><div style={{ display:"grid",gap:12,marginTop:22 }}>{question.options.map((option)=>{const isCorrectOption=submitted[question.id]&&option===question.answer;const isWrongSelection=submitted[question.id]&&option===selected&&option!==question.answer;return <button key={option} type="button" onClick={()=>choose(option)} style={{ textAlign:"left",padding:"15px 18px",borderRadius:12,border:`2px solid ${isCorrectOption?"var(--blue)":selected===option?"var(--blue)":"var(--border)"}`,background:isCorrectOption?"rgba(0,59,147,.10)":isWrongSelection?"rgba(200,0,0,.08)":selected===option?"rgba(0,59,147,.07)":"var(--card)",color:"var(--text)",cursor:isAnswered||isExpired?"default":"pointer",fontSize:16 }}>{option}</button>})}</div>{!isAnswered&&!isExpired&&<button type="button" disabled={!selected} onClick={submit} style={{ marginTop:24,padding:"13px 20px",border:0,borderRadius:10,background:"var(--blue)",color:"#fff",fontWeight:800,opacity:selected?1:.55 }}>Check answer</button>}{(isAnswered||isExpired)&&<div style={{ marginTop:22,padding:18,borderRadius:12,background:"rgba(0,59,147,.07)" }}><strong>{isExpired?`Time expired. The answer is ${question.answer}.`:correct?"Correct.":`Not quite. The answer is ${question.answer}.`}</strong><p style={{ marginBottom:0,lineHeight:1.6 }}>{question.explanation}</p></div>}<div style={{ display:"flex",justifyContent:"space-between",gap:12,marginTop:28 }}><button type="button" onClick={previous} disabled={currentIndex===0} style={{ padding:"12px 18px",borderRadius:10,border:"1px solid var(--border)",background:"var(--card)",color:"var(--text)",opacity:currentIndex===0 ? .45 : 1 }}>Previous</button>{currentIndex<questions.length-1?<button type="button" onClick={next} disabled={!isAnswered&&!isExpired} style={{ padding:"12px 18px",borderRadius:10,border:0,background:"var(--blue)",color:"#fff",fontWeight:800,opacity:!isAnswered&&!isExpired ? .5 : 1 }}>Next</button>:<button type="button" onClick={finish} disabled={handledCount<questions.length} style={{ padding:"12px 18px",borderRadius:10,border:0,background:"var(--blue)",color:"#fff",fontWeight:800,opacity:handledCount<questions.length ? .5 : 1 }}>Complete activity</button>}</div></section> : <section style={{ marginTop:30,padding:32,borderRadius:18,background:"var(--card)",border:"1px solid var(--border)",textAlign:"center" }}><div style={{ color:"var(--blue)",fontWeight:800,textTransform:"uppercase",letterSpacing:".08em",fontSize:13 }}>Activity complete</div><h2 style={{ fontSize:34,margin:"10px 0" }}>Score: {score}%</h2><p style={{ color:"var(--muted)",lineHeight:1.6 }}>Your Level 1 {activity.half} activity has been recorded in Reasoning progress.</p><div style={{ display:"flex",justifyContent:"center",gap:12,flexWrap:"wrap",marginTop:18 }}><Link href={moduleHref} style={{ display:"inline-block",padding:"12px 18px",borderRadius:10,background:"var(--blue)",color:"#fff",textDecoration:"none",fontWeight:800 }}>Back to {module ? module.title : `${trackName} Stage 1`}</Link><Link href={`/Reasoning/${trackName}/Dashboard/Stage1`} style={{ display:"inline-block",padding:"12px 18px",borderRadius:10,border:"1px solid var(--border)",color:"var(--text)",textDecoration:"none",fontWeight:800 }}>Back to Stage 1</Link></div></section>}
  </div></main></>;
}
