...
import { getQuestionsForActivity, getDefaultTimeSeconds } from "../../../Data/Reasoning/questionBank";
...
export default function ReasoningActivity(){
 ...
 const activity=useMemo(()=>{const rawId=router.query.activityId;const id=Array.isArray(rawId)?rawId[0]:rawId;return getActivityById(id)||getStage4ActivityById(id)||getStage5ActivityById(id)||getStage6ActivityById(id)},[router.query.activityId]);
 ...
 const questions=useMemo(()=>{
  ...
  const baseQuestions=stage===1?getQuestionsForActivity(activity.track,activity.levelId,activity.half,activity.id):stage===2?getStage2QuestionsForActivity(activity.id):getStage3QuestionsForActivity(activity.id);
  const extensionQuestions=stage===1?getStage1ExtensionQuestionsForActivity(activity.id):[];
  return uniqueQuestions([...baseQuestions,...extensionQuestions]);
 },[activity,stage]);
 const moduleId=useMemo(()=>{if(!activity)return null;if(stage===1)return findModuleId(STAGE1_MODULES,activity.id)||activity.moduleId;if(stage===2)return findModuleId(STAGE2_MODULES,activity.id)||activity.moduleId;if(stage===3)return findModuleId(STAGE3_MODULES,activity.id)||activity.moduleId;if(stage===5)return findModuleId(STAGE5_MODULES,activity.id)||activity.moduleId;if(stage===6)return findModuleId(STAGE6_MODULES,activity.id)||activity.moduleId;return activity.moduleId||null},[activity,stage]);
 ...
 const moduleHref=moduleId&&stage!==4?`/Reasoning/${trackName}/Dashboard/Stage${stage}/${moduleId}`:stageHref;
 ...
 const difficultyValue=typeof question?.difficulty==="number"?question.difficulty:({easy:1,medium:2,hard:3,core:1,stretch:3,advanced:4}[question?.difficulty]||1);
 const rawTime=question?.timePerQuestion;
 const timeLimit=question?(Number.isFinite(Number(rawTime))?Math.max(1,Number(rawTime)):getDefaultTimeSeconds({levelId:question.levelId,difficulty:difficultyValue,questionType:question.questionType})):0;
 ...
 useEffect(()=>{if(!question||isAnswered||isExpired||completed)return;setRemaining(timeLimit);const timer=window.setInterval(()=>setRemaining(value=>{if(value<=1){window.clearInterval(timer);setExpired(current=>{const next={...current,[question.id]:true};if(currentIndex===questions.length-1&&!completed&&user?.userId){const finalScore=Math.round((questions.reduce((n,q)=>n+(submitted[q.id]===q.answer?1:0),0)/questions.length)*100);completeReasoningActivity(user.userId,activity.track,activity.id,finalScore);setCompleted(true)}return next});if(currentIndex<questions.length-1)setCurrentIndex(index=>index+1);return 0}return value-1}),1000);return()=>window.clearInterval(timer)},[question?.id,timeLimit,isAnswered,isExpired,currentIndex,questions.length,completed,user?.userId,activity?.id,activity?.track,submitted]);
