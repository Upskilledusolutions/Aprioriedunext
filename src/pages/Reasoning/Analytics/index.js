...
const styles = {
  ...
  barActivityLabel: { position: "absolute", right: "calc(50% + 3px)", bottom: "calc(100% - 4px)", transform: "rotate(-45deg)", transformOrigin: "right bottom", whiteSpace: "nowrap", color: "#7f8da1", fontSize: 7, lineHeight: 1.1, fontWeight: 500 },
    ...
    <section style={styles.barPanel} aria-label={`${title} activity performance scores`}>
      <div style={styles.barPanelHeader}>
        <div>
          ...
          <p style={styles.barPanelSubtitle}>Performance score (%) by activity · NA = no recorded score</p>
        </div>
      </div>
          ...
          <div style={styles.barGridArea}>
            <div style={styles.barItems}>
              {data.map((item) => {
                ...
                const accessibleValue = hasScore ? item.score + "%" : "NA";
