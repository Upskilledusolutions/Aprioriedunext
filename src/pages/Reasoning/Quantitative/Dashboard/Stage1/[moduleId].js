...
import { getActivities, getActivityById } from "../../../../../Data/Reasoning/activities";
...
export default function QuantitativeModule() {
  ...
  const allStageActivities = getActivities({ track: "quantitative", levelId: "L1", stageId: "S1" });
  ...
  return <>
    ...
    <main style={{ minHeight: "80vh", padding: "48px 20px 80px", background: "var(--surface)" }}>
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <section style={{ marginTop: 30, padding: 28, borderRadius: 18, background: "var(--card)", border: "1px solid var(--border)" }}>
          ...
          <section style={{ marginTop: 32 }}>
            ...
            {activities.length ? activities.map((activity) => {
              const done = Boolean(progress.quantitative?.completedActivities?.[activity.id]);
        ...
        <p style={{ marginTop: 20, color: "var(--muted)", fontSize: 13 }}>Stage activity library: {allStageActivities.length} activities. Module activity links are connected through the reusable Reasoning activity system.</p>
      </div>
    </main>
  </>;
}
