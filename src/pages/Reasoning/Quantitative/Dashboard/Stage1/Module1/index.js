...
const questions = [
  {
    prompt: "Which estimate is closest to 49 × 21?",
    options: ["500", "1,000", "1,500", "2,000"],
    answer: 1,
    explanation: "49 is close to 50 and 21 is close to 20, so 50 × 20 = 1,000 is a strong estimate.",
  },
  {
    prompt: "Which number is greatest?",
    options: ["0.72", "7/10", "69%", "0.705"],
    answer: 0,
    explanation: "0.72 = 72%, which is greater than 70%, 69% and 70.5%.",
  },
  {
    prompt: "A quantity increases from 80 to 100. What is the percentage increase?",
    options: ["20%", "25%", "30%", "40%"],
    answer: 1,
    explanation: "The increase is 20. Relative to the original 80, 20 ÷ 80 = 25%.",
  },
  {
    prompt: "Which statement must be true if x is greater than 12?",
    options: ["x is negative", "x is less than 10", "x is greater than 10", "x equals 12"],
    answer: 2,
    explanation: "Any number greater than 12 is necessarily greater than 10 as well.",
  },
  {
    prompt: "A calculator gives 2.4 × 49 = 117.6. Which quick check is most useful?",
    options: ["49 is about 50, so the product should be about 120", "The answer must be below 50", "2.4 + 49 should equal 117.6", "The product should be exactly 100"],
    answer: 0,
    explanation: "Rounding 49 to 50 gives 2.4 × 50 = 120, so 117.6 is plausible.",
  },
];
...
const styles = {
  ...
  option: { display: "block", width: "100%", textAlign: "left", padding: "14px 16px", marginTop: 11, borderRadius: 11, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", cursor: "pointer", fontSize: 16 },
...
export default function NumberSenseModule() {
  ...
  if (finished) {
    return (
      <>
        <Head><title>Number Sense Complete | Quantitative Skills | Upskilleduonline</title></Head>
        <main style={styles.page}>
          <div style={styles.wrapper}>
            <Link href="/Reasoning/Quantitative/Dashboard/Stage1" style={styles.back}>← Back to Foundation Quantitative</Link>
            <section style={styles.card}>
              <p style={styles.eyebrow}>Module complete</p>
              <h1 style={styles.resultTitle}>Number Sense</h1>
              ...
              <div style={styles.actions}>
                <Link href="/Reasoning/Quantitative/Dashboard/Stage1/Module1" style={styles.primary}>Try Again</Link>
                <Link href="/Reasoning/Quantitative/Dashboard/Stage1" style={styles.secondary}>View Stage Progress</Link>
              </div>
            </section>
          </div>
        </main>
  ...
  return (
    <>
      <Head>
        <title>Number Sense | Quantitative Skills | Upskilleduonline</title>
        <meta name="description" content="Interactive Number Sense practice for foundational quantitative reasoning." />
      </Head>
      <main style={styles.page}>
        <div style={styles.wrapper}>
          <Link href="/Reasoning/Quantitative/Dashboard/Stage1" style={styles.back}>← Back to Foundation Quantitative</Link>
          ...
          <p style={styles.intro}>Predict first, calculate second, check third. Work through five questions and use the explanations to strengthen your quantitative intuition.</p>

          <section style={styles.progress}>
            <div style={styles.progressTop}><span>Question {current + 1} of {questions.length}</span><span>{score} correct</span></div>
            <div style={styles.track}><div style={{ ...styles.fill, width: `${((current + (answered ? 1 : 0)) / questions.length) * 100}%` }} /></div>
          </section>

          <section style={styles.card}>
            <h2 style={styles.question}>{question.prompt}</h2>
            {question.options.map((option, index) => (
              <button key={option} type="button" onClick={() => handleAnswer(index)} style={{ ...styles.option, ...(selected === index ? styles.selected : {}) }}>
            ...
            {answered && (
              <div style={styles.feedback}>
                ...
                <div style={styles.actions}>
                  <button type="button" onClick={handleNext} style={styles.primary}>{current === questions.length - 1 ? "Complete Module" : "Next Question"}</button>
                </div>
              </div>
            )}
          </section>
