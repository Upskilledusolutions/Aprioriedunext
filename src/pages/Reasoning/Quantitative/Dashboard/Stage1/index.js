...
export default function FoundationQuantitative() {
  ...
  return (
    <>
      <Head>
        <title>Foundation Quantitative & Reasoning | Upskilleduonline</title>
        <meta name="description" content="Build number sense, patterns, logic and foundational quantitative reasoning." />
      </Head>
          ...
          <div style={styles.grid}>
            {modules.map(([number, title, text, action], index) => (
              <article key={number} style={styles.card}>
                ...
                <Link href={`/Reasoning/Quantitative/Dashboard/Stage1/Module${index + 1}`} style={styles.button}>{action}</Link>
              </article>
            ))}
          </div>
