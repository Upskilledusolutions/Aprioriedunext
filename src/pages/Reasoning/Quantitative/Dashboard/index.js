...
export default function QuantitativeDashboard() {
  ...
  const completedCount = getTrackCompletedCount(progress, "quantitative", 5);
  const percent = getTrackPercent(progress, "quantitative", 5);
  const nextModule = completedCount === 0 ? 1 : Math.min(completedCount + 1, 5);

  return (
    <>
      <Head><title>Quantitative Skills Dashboard | Upskilleduonline</title><meta name="description" content="Build quantitative reasoning from foundations through mathematical research." /></Head>
      <main style={styles.page}><div style={styles.wrapper}>
        ...
        <div style={styles.header}><p style={styles.eyebrow}>Quantitative Skills{user?.name ? ` · ${user.name}` : ""}</p><h1 style={styles.title}>Think mathematically. Solve unfamiliar problems.</h1><p style={styles.intro}>Your six-stage pathway is organized from foundational reasoning through advanced problem solving, proof and research.</p></div>
        <section style={styles.progress}><div style={styles.progressTop}><span>Overall quantitative progress</span><span>{percent}% · {completedCount}/5 Stage 01 modules</span></div><div style={styles.track}><div style={{ ...styles.fill, width: `${percent}%` }} /></div>{completedCount < 5 && <Link href={`/Reasoning/Quantitative/Dashboard/Stage1/Module${nextModule}`} style={styles.continue}>Continue Learning →</Link>}</section>
        <div style={styles.grid}>{stages.map(([number, title, text], index) => <section key={number} style={styles.card}><div style={styles.number}>STAGE {number}</div><h2 style={styles.cardTitle}>{title}</h2><p style={styles.text}>{text}</p><span style={styles.status}>{index === 0 ? `${completedCount === 0 ? "Ready to start" : `${completedCount}/5 modules complete`}` : "Locked until pathway is built"}</span>{index === 0 && <Link href="/Reasoning/Quantitative/Dashboard/Stage1" style={styles.action}>Open Foundation Stage →</Link>}</section>)}</div>
      </div></main>
    </>
  );
}
